...
$.fn.sleekslider = function(object) {
	...
	if (object.thumbs != undefined || object.thumbs != null) {
		if (object.thumbs.length > 1) {
			...
			$prevThumbnail.attr('src', 'fornt/images/' + thumbs[thumbs.length - 1]);
			$prevText.text(labels[thumbs.length - 1]);

			$nextThumbnail.attr('src', 'fornt/images/' + thumbs[active]);
...
function animateSlider() {
	...
	if (thumbs != undefined || thumbs != null) {
		if (prevClicked) {
			...
			$prevThumbnail.attr('src', 'fornt/images/' + thumbs[value]);
			$prevText.text(labels[value]);
		} else {
			$prevThumbnail.attr('src', 'fornt/images/' + thumbs[active - 1]);
			$prevText.text(labels[active - 1]);
		}

		if (active === (thumbs.length - 1)) {
			$nextThumbnail.attr('src', 'fornt/images/' + thumbs[0]);
			$nextText.text(labels[0]);
		} else {
			$nextThumbnail.attr('src', 'fornt/images/' + thumbs[next]);
